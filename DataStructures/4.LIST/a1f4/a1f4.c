...
int main(){
    ...
    ListPointer List, frptr, predptr;
    int i, number;
    char answer;

/*-----A-----*/
    InitializeStoragePool(Node, &frptr);
...
/*-----E-----*/
    ...
    do
    {
        ...
        scanf("%d", &number);
        getchar();
        if(!Search(frptr, List, Node, number, &predptr)){
            Insert(&List, Node, &frptr, predptr, number);
        }
        do
        {
            printf("Continue Y/N: ");
            scanf("%c", &answer);
            getchar();
        } while ('N' != toupper(answer) && 'Y' != toupper(answer));
...
boolean Search(ListPointer FreePtr, ListPointer List, NodeType Node[NumberOfNodes], ListElementType Item, ListPointer *PredPtr)
{
    //boolean stop = FALSE;while(curr != NilValue && !stop){if(Node[curr].Data >= Item){stop = TRUE;found = Node[curr].Data == Item;}else{*PredPtr = curr;curr = Node[curr].Next;}}
    boolean found = FALSE;
    ListPointer curr;
